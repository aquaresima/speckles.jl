...
fits_analysis = load(joinpath(analysis_path,savename("$filename", config, "jld2"))) |> dict2ntuple
video_props = load(joinpath(analysis_path,"videos_properties.jld2")) |> dict2ntuple
correlations = load(joinpath(analysis_path,"$(filename)_correlations.jld2")) |> dict2ntuple
fits = load(joinpath(analysis_path,"$(filename)_fits.jld2")) |> dict2ntuple

@unpack speeds, times = correlations
...
default(guidefontsize=18, tickfontsize=11, titlefontsize=15)
...
heatmap(norm .* mean(shift_mat(_mat), dims=3)[:,:,1], c=:haline, xticks=(0:50:250, 0:2:10), yticks=(0:100:400, reverse(0:4:16)), clims=(50,256))
##
...
for (file,speed) in collect(zip(files, speeds))[[1,3,6,9]]
    ...
    push!(plots, heatmap(flat, title="ω = $speed (s⁻¹)", c=:haline,titlefontsize=14,xticks=(0:80:240, 0:80:240),yticks=(0:100:400, reverse(0:100:400)), clims=(50,256)))
	if length(plots) <6
		plot!(colorbar=false)
	end
end
...
p = heatmap(reverse(z), ymirror=true, yticks=(0:50:256, 0:50:256), c=:haline, cbar=false, xticks=:none, ylabel=L"\langle"*"Pixel intensity"*L"\rangle", box=:on,grid=:off)
plot!(plots[3], ylabel="                   z (px)", xlabel="                            width (px)")
